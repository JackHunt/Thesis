...
system itself is based on Monocular EKF SLAM \cite{EKFSLAM} with semantics added to points via 
correspondences between SURF\cite{SURF} image features extracted from the observed RGB frames and 
...
are detected using a Random Forest (RF) \cite{RF} classifier trained on hand crafted features 
...
and CIELab \cite{CIELAB} appearance information.
...
Fully Connected Network (FCN) \cite{FCN}, taking the Probabilituy Mass Function (PMF) output to 
determine the class to be assigned to an isosurface region.
...
automatically. The authors utilise Maximum Flow \cite{MAXFLOW} to solve a discretised flow field cost 
...
However, contrary to \textit{Sun et al} \cite{Sun2012} who utilise a Maximum Flow algorithm over the 
...
segmentations and parameters being solve for within an Expectation Maximisation (EM) \cite{EM} 
framework. The presented approach is robust to both simultaneous foreground and background motion 
by giving each parity.
...
have advantages of it's highly optimised volumetric counterparts, such as that proposed by 
...
of interest. Motion segmentation is posed as a Maximum a Posteriori (MAP) \cite{MAP} inference 
problem over a Conditional Random Field (CRF) \cite{CRF} defined over TSDF voxels. The proposed 
system is able to segment objects undergoing both minor and major displacements, with motion labels 
and parameters found with respect to the live frame and the TSDF. However, the proposed approach is 
limited only to very small scenes, with very long runtimes reported for TSDF volumes of dimensionality 
$256^{3}$.
...
estimation of a 6DoF motion field that warps model represented by the TSDF to the live frame. The 
...
as-rigid-as-possible \cite{ARAP} manner. However, the specification of object rotation in front of a 
...
and shape deformations by utilising bundle adjustment \cite{BA}. The authors report that loop closures 
...
Graph Cuts \cite{GC} and Alpha Expansions \cite{AE}. The proposed approach utilises 
a photometric loss to optimise for object pose, with keyframe based loop closure detection. However, the 
authors report difficulties in building sufficiently granular reconstructions. In addition, the authors 
report drift in pose estimation to be problematic.
...
Total Variation (TV) \cite{TV} regularisation. However, there is no pose estimation 
...
utilising cascaded Deep Neural Network (DNN) \cite{DNN} regressors. The authors utilise the DNN framework 
for the complex task of articulated human pose estimation.
...
Network (CNN) \cite{CNN} within a nearest neighbour cost function for object detection and recovery of 
rough pose. As such, the proposed approach poses the problem as a K-Nearest Neighbour (KNN) \cite{KNN} 
search in descriptor space. Object and pose are coupled in training(i.e. two similar cars with different 
poses will have spatially distant descriptors).
...
database of a priori known objects by the use of a Random Forest \cite{RF}. The matched shapes are coarsly 
matched to the input depth map, then later deformed at a higher granularity by a separate optimisation 
process.
...
Network (RPN) \cite{RPN}. The authors evaluate the proposed approach on the KITTI LIDAR \cite{Geiger2013} 
...
(GAN) \cite{GAN} like approach to shape prediction. The proposed network is trained in a weakly supervised 
manner on silhouettes and 3D shapes with a log-barrier \cite{LB} objective function. However, the 
appliability to ``real world'' scenarios is questionable, due to the synthetic nature of the data used 
to train and evaluate the network.
