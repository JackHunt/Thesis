...
infancy. This advancement is most predominant within the sense SLAM literature. 
This section shall first explore the earlier, fundamental works of this area of research, 
followed by a treatment of the current state of the art.
...
components with attributes such as surface normal vectors, confidences and RGB colour. The motivation 
...
\textit{Curless \& Levoy}~\cite{Curless1996}; the Signed Distance Function. Additionally, a central theme 
in the work that follows is pose estimation, specifically utilising variants of the Iterative Closest Points 
(ICP) algorithm, as introduced by \textit{Besl \& McKay}~\cite{Besl1992}. Later work on the \textit{KinectFusion} 
...
process are named \textit{Voxel Oriented Patch} features and consist of surface normal vectors
...
Fully Connected Network (FCN)~\cite{Long2015}, taking the Probability Mass Function (PMF) 
output to determine the class to be assigned to an isosurface region.
% TODO: semanticfusion, matthias stuff
...
by the Approximate Nearest Neighbour (ANN)~\cite{Indyk2000} algorithm.
...
% TODO: Fill this section out a bit.
