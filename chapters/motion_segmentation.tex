...
\subsubsection{Stability Labelling}
\label{subsub:moseg_stability_labelling}
...
\begin{equation}
  ...
  \bigg|\Phi(v)^{d} - \text{min}\bigg(1, \frac{\eta}{\mu}\bigg)\bigg| \Bigg]
\end{equation}
...
\begin{equation}
  ...
  \tau_{\mathcal{V}} =
  \begin{cases}
    \tau_{\mathcal{V}} + 1 & \textbf{if} \mkern4mu l_{\mathcal{V}} = \text{stable}\\
...
time (empirically set to $40$ frames) will be fused into the Static Model,
as follows in Section \ref{subsub:moseg_static_to_dynamic}.

\subsubsection{Integration From Dynamic to Static Models}
\label{subsub:moseg_static_to_dynamic}
