...
CRF~\cite{BishopPRML} framework. Textured renderings of various objects reconstructed 
with the proposed system are given in Figure~\ref{figure:probobj_teaser}.

\begin{figure}[!htbp]
  \centering
  \begin{tabular}{ccc}
    \includegraphics[width=.3\linewidth]{figures/object_recon/colour/bear.png} &
    \includegraphics[width=.3\linewidth]{figures/object_recon/colour/chair.png} &
    \includegraphics[width=.3\linewidth]{figures/object_recon/colour/cabinet.png} \\
    (a) & (b) & (c) \\
  \end{tabular}
  \caption[Textured Object Reconstructions]
  {
    \begin{tabular}[t]{@{}l@{}}
      (a) Textured reconstruction of a Teddy Bear. \\
      (b) Textured reconstruction of a Chair. \\
      (c) Textured reconstruction of a Filing Cabinet.
    \end{tabular}
  }
~\label{figure:probobj_teaser}
\end{figure}
...
volume of probabilities, pertaining to posteriors over a voxel's assignment to 
...
generate an object probability map defining the segmented region to be
...
the system accumulates the probabilities into the object probability volume
of the active sub-volume. Examples of such instantaneous probability maps are given in 
Figure~\ref{figure:probobj_prob_maps}.
\begin{figure}[!htbp]
  \centering
  \begin{tabular}{cc}
...
The representation of the reconstructed object comprises multiple \textit{sub-volumes}, 
...
\begin{figure}[!htbp]
  ...
  \begin{tabular}{cc}
    ...
    (c) & (d) \\
  \end{tabular}
  \caption[Subvolume Examples]
  {
    Example subvolume renderings of a given object of interest. As viewed from 
    the front:\\
    \begin{tabular}[t]{@{}l@{}}
...
~\label{figure:probobj_subvolumes}
\end{figure}
...
To encode the probability of a given voxel \( \psi \in \Psi \) containing an
...
Given the CDF derived in Equation~\ref{eqn:probobj_surface_prior_cdf}, 
...
\subsection{Implicit Surface Deformation}
      ...
      \begin{algorithmic}[1]
        ...
        \Procedure{Object Reconstruction Iteration}{$\bm{\Omega}, \mathcal{S}, u, t$}
          ...
          \For{\( s \in \mathcal{S} \)} \Comment{For Each Submap \( s \)}
            \If{\( \text{isVisible}(s) \)}
              \State{\( \Phi_{s} \gets \text{getVolume}(s) \)}
              \State{\( \bm{T}^{s}_{t} \gets \text{getPose}(s) \)}
              
              \State{\( \mathcal{R}_{s} \gets \text{raycast}(\Phi_{s}, \bm{\Omega}_{depth}, \bm{T}^{s}_{t}) \)}
              ...
              \State{\( \Phi_{s} \gets \text{integrate}(\mathcal{D}, \Phi_{s}, \bm{T}^{s}_{t+1})\)}
          ...
          \For{\( s,s^{'} \in \mathcal{S} \)} \Comment{For Overlapping Submaps \( s,s^{'} \)}
            ...
            \State{\( \bm{L}^{\star}_{s, s^{'}} \gets \text{inferOptimalConstraints}(\Phi_{s}, \Phi_{s^{'}}, \bm{L}_{s, s^{'}}) \)}
...
\begin{figure}[!htbp]
  ...
  \caption[Probabilistic Object Reconstruction Qualitative Results I]
  {
    \begin{tabular}[t]{@{}l@{}}
      ...
      from\\ RGBD observations on the Museum Rock sequence.\\ 
      (b) The proposed system (of this work) reconstructing 
      the same object\\ from RGBD observations of the Museum Rock sequence.
    \end{tabular}
  }
...
\begin{figure}[!htbp]
  ...
  \caption[Probabilistic Object Reconstruction Qualitative Results II]
  {
    \begin{tabular}[t]{@{}l@{}}
      (a) The system of \textit{Ren et al}~\cite{Ren2013} evolving a shape prior SDF 
      from RGBD\\ observations on the Museum Dinosaur Head sequence.\\ 
      (b) The proposed system (of this work) reconstructing same the object\\ from RGBD observations of the 
      Museum Dinosaur Head sequence.
    \end{tabular}
  }
...
\begin{figure}[!htbp]
  ...
  {
    \begin{tabular}[t]{@{}l@{}}
      Comparisons of object reconstructions, versus manually segmented (after\\ scene reconstruction) 
      ground truth models:\\
      (a, b) Dinosaur Head.\\
      (c, d) Chair.\\
      (e, f) Bear.\\
...
As can be seen in Figures~\ref{figure:probobj_comp_itm} and~\ref{figure:probobj_comp_itm2}, 
the proposed system is capable of providing reconstructions of a wide variety of objects in an 
...
the top-down views presented in Figures~\ref{figure:probobj_comp_itm} and~\ref{figure:probobj_comp_itm2}.
\begin{figure}[!htbp]
	\centering
	\begin{tabular}{cc}
  ...
  {
    \begin{tabular}[t]{@{}l@{}}
      Closed reconstructions of the following sequences: 
      (a) Teddy.\\
      (b) Dinosaur Head.\\
      (c) Chair.\\
      (d) Rock sequences.
...
~\label{figure:probobj_comp_itm2}
\end{figure}
...
demonstrates the difference in reconstruction output when not utilising the multiple sub-volume representation 
...
\begin{figure}[!htbp]
  ...
  {
    \begin{tabular}[t]{@{}l@{}}
      Teddy reconstruction with InfiniTAM versus the system proposed in this work:\\
      (a, b) InfiniTAM.\\
...
A final set of examples of the system in use for face reconstruction are given in 
Figure~\ref{figure:probobj_face}, the objective of which is to demonstrate tracking with 
mostly uniform appearance.

\begin{figure}[!htbp]
	\centering
	\begin{tabular}{cc}
		\includegraphics[width=.4\linewidth]{figures/object_recon/face/0.png}&
    \includegraphics[width=.4\linewidth]{figures/object_recon/face/1.png}\\
    (a) & (b) \\
		\includegraphics[width=.4\linewidth]{figures/object_recon/face/2.png}&
    \includegraphics[width=.4\linewidth]{figures/object_recon/face/3.png}\\
    (c) & (d)\\
	\end{tabular}
  \caption[Probabilistic Object Reconstruction Qualitative Results V]
  {Tracking and reconstructing a face at various poses.}
~\label{figure:probobj_face}
\end{figure}
...
yields a marked improvement in ATE over that of the standard KinectFusion pipeline with object 
segmentation. 

The remaining sequences in the \textit{3D Object Reconstruction} subset of the 
\textit{RGB-D SLAM Dataset and Benchmark}~\cite{Sturm2012} did not yield an interpretable reconstruction; 
these sequences did not reconstruct in \textit{neither} the proposed approach, \textit{nor} the baseline 
approach (InfiniTAM). In the case of the \textit{freiburg2\_metallic\_sphere (2)} sequences, the object 
...
\textit{freiburg2\_coke} sequence in \textit{both} systems. The \textit{freiburg2\_flowerbouquet (brownbg)} 
sequences also proved troublesome for \textit{both} approaches. When inspecting the depth stream for this 
sequence it is apparent that much of the flower bouquet does not have consistent depth data, in some cases 
...
The trajectories of the proposed object reconstruction system are evaluated on the \textit{freiburg3\_cabinet} 
and \textit{\textsf{freiburg3\_teddy}} sequences of Table~\ref{table:probobj_ate}, and are plotted versus the 
ground truth trajectories in Figure~\ref{figure:probobj_traj}.
\begin{figure}[!htbp]
  \centering
  \begin{tabular}{@{}l@{}}
  \includegraphics[width=.8\linewidth]{figures/object_recon/plots/traj/cab_traj.eps}\\
  (a)\\
  \includegraphics[width=.8\linewidth]{figures/object_recon/plots/traj/ted_traj.eps}\\
  (b)
  \end{tabular}
  \caption[Probabilistic Object Reconstruction Trajectory Plots]
  {
    \begin{tabular}[t]{@{}l@{}}
    Trajectory plots for the following sequences:\\
    (a) \textit{freiburg3\_cabinet}.\\
    (b) \textit{\textsf{freiburg3\_teddy}}.
    \end{tabular}
  }
...
\begin{figure}[!htbp]
  ...
  {
    \begin{tabular}[t]{@{}l@{}}
      (a, b, c, d) Renderings of the sequences evaluated in Table~\ref{table:probobj_hauss} textured\\ 
      on Hausdorff Distance.\\
      (e) Intensity mapping corresponding to the minimum and maximum\\ values given in Table~\ref{table:probobj_hauss}.
    \end{tabular}
  }
...
It can be seen in Figure~\ref{figure:probobj_timing} that for the \textit{Bear} and \textit{Museum Rock} 
sequences that there is a multimodality in the timing distributions. On further analysis it is apparent 
that this is due to the non-consistent sizing of the reconstructed subvolumes. Thus, a subvolume with 
high occupancy in the view frustum requires more processing time due to the increase of surface data. 
This is apparent when a new subvolume is created when viewing a smaller portion of an object. For example, 
if a new subvolume is created whilst viewing the side of the \textit{Bear}, that subvolume will naturally 
contain a larger amount of distance and surface data than the case of viewing from the front, for example.
