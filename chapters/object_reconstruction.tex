...
During the fusion process, the Probability Density Functions (PDF) or Probability Mass Functions (PMF) of 
...
\begin{figure}[!htbp]
  % TODO: Try and do something with this image. Too small to read text.
  \centering
...
\begin{align}
  ...
  \frac{1}{\left| \mathcal{X} \right|} \sum_{\chi \in \mathcal{X}} 
    \begin{aligned}[t]
      {}& \Big[ 
        \bm{\Psi}_{s}(v) - \bm{\Psi}_{s^{'}}(v^{'})
        \forall v, v^{'} \in \bm{\Psi}_{s} \cup \bm{\Psi}_{s^{'}}
        \leftrightarrow \\ 
      {}& 1 - P(\bm{\Psi}_{s}(v) \in \mathbf{\Phi} | \Omega, p) < 0.5 \wedge \\
      {}& 1 - P(\bm{\Psi}_{s^{'}}(v^{'}) \in \mathbf{\Phi} | \Omega, p) < 0.5
      \Big]
    \end{aligned}
\end{align}
