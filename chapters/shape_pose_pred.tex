...
% takes a stereo pair and extracts features
% features regress latent space shape point and pose
% shape generated from gp prior conditioned on latent space point
% evaluated by rendering and quantifying fitness with KL div
% fully differentiable - diff raycast - standard backpropagation and others e.g. adam
% unsupervised
...
% modelling shape by assuming geomeetry is derived from LV's
% finds linear mapping from observed data space to hidden latent space
% equivelant to probabilistic pca in the linear case
% bayesian formulation allows for marginalisation of latent variables
% gplvm framework allows for complex, nonlinear modelling with kernels after marginalisation
% places a gaussian process prior over function space
% optimisation is non convex

\subsubsection{Gaussian Process Marginal Likelihood}
~\label{subsubsec:spp_gp_marginal_likelihood}
% linear mapping is not observable, thus latent
% tractable by deriving marginal likelihood, integrate over mapping
% likelihood and prior gaussian; conjugate prior
...
% form of above outlines...
% posterior mean corresponds 20 N^3 voxel grid
% obtained via gaussian process regression
% observed latent point conditioned on GP prior
% granularity of training controlled by DCT, thus needs IDCT on regression, N harmonics - carl paper
...
% model trained on sdf's compressed by DCT
% efficacy in shape regression yada yada (carl)
% GIVE DCT
