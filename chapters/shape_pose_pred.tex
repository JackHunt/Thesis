...
given framework allows for complex, nonlinear embeddings to be learnt via the use of kernel functions, 
...
the solution space is broad, it is neccessary to quantify the model's performance.

Given in Figure~\ref{figure:spp_training} is the model's loss, quantified by Equation~\ref{eqn:spp_overall_loss} over a trainining session 
of \( 500,000 \) examples, utilising the Adam Optimizer~\cite{Diederik2014} and a learning rate of \( 0.0001 \).
\begin{figure}[!htbp]
  \centering
...
Firstly, as CNN models are notouriously difficult to train~\cite{Glorot2010}, it is plausible that during training, 
...
``learning'' a sufficiently general functional mapping, to unsuitable nonlinear activations. Furthermore, the 
...
predictions in a manner that is conducive to model convergence. Note that if a predicted pose causes the raycasting 
