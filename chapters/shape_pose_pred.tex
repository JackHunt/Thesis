...
is the use of stereo RGB frames versus RGBD\@. The reason for the change in input data format 
is twofold. Firstly, the use of RGBD is prohibitive when considering outdoor environments where 
lighting conditions may prevent RGBD sensors such as the Microsoft Kinect from producing a meaningful 
depth map. Second, is the intuition that utilising multiple views of an object (from each sensor in 
the stereo pair) will make the pose estimation more amenable to learning based methods.
...
under the generated pose and computing an information theoretic loss between the image statistics of 
